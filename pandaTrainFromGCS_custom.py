...
with strategy.scope():
    Q = tf.Variable(tf.random.normal([20,15]), trainable=True)

    inputs = tf.keras.layers.Input(shape=(17280000,))
    reshape = tf.keras.layers.Reshape((1500,3,3,1280))(inputs)
    conv3d = tf.keras.layers.Conv3D(15, kernel_size=(1500,3,3), activation='relu')(reshape[:,:,:,:,:])
    #attn = tf.keras.layers.Attention()([Q,conv3d[:,0,0,0,:]])
    flat = tf.keras.layers.Flatten()(conv3d[:,0,0,0,:]) #(attn)
    ...
    manager = tf.train.CheckpointManager(ckpt, 'gs://awxyz-us-central1/checkpoints/20x15_custom', max_to_keep=3)
    ...
    writer = tf.summary.create_file_writer("gs://awxyz-us-central1/checkpoints/20x15_custom/logs")
